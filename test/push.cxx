...
TEST_F(Opcode_PUSH_Test, PUSH_BC_Test) {
  ...
  memory_t program = {LD_SP_D16, 0xfe, 0xff, LD_BC_D16, 0xcd, 0xab, PUSH_BC};
  ...
  EXPECT_EQ(mem[regs.SP], 0xcd);
  EXPECT_EQ(mem[regs.SP + 1], 0xab);
}

TEST_F(Opcode_PUSH_Test, PUSH_DE_Test) {
  /** start - inline program */
  memory_t program = {LD_SP_D16, 0xfe, 0xff, LD_DE_D16, 0xcd, 0xab, PUSH_DE};
  memory.load_memory(program);
  cycles_t steps = 10;
  /** end   - inline program */
...
TEST_F(Opcode_PUSH_Test, PUSH_HL_Test) {
  /** start - inline program */
  memory_t program = {LD_SP_D16, 0xfe, 0xff, LD_HL_D16, 0xcd, 0xab, PUSH_HL};
  memory.load_memory(program);
  cycles_t steps = 10;
  /** end   - inline program */
...
TEST_F(Opcode_PUSH_Test, PUSH_AF_Test) {
  /** start - inline program */
  memory_t program = {LD_SP_D16, 0xfe, 0xff, LD_A_D8, 0xab, PUSH_AF};
  memory.load_memory(program);
